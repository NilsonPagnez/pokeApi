...
function primeiraLetraMaiuscula(PrimeLetra) {
  return PrimeLetra.charAt(0).toUpperCase() + PrimeLetra.slice(1);
}


function createCardPokemon(code, type, nome, imagePok){ //criar os cards dos pokemons via JS
  ...
  codePokemon.textContent = (code < 10 ) ? `#00${code}`: (code < 100) ? `#0${code}` : `#${code}`;//if ternário
...
function openDetailsPokemon() {
  ...
  axios({
    method: 'GET',
    url: `http://pokeapi.co/api/v2/pokemon/${codePokemon}`
  })
  .then(response =>{
...
//Função para filtrar pokemon por tipo
...
function searchPokemon(){
  ...
  .catch((error)=>{ //se der erro vai aparecer isso daqui
    if(error.response) {
